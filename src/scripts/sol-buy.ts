...
import { PRIVATEKEY, UXDACCOUNT, WSOLACCOUNT } from "./.env";
import * as bs58 from "bs58";

async function main() {
  const mainAccount = Keypair.fromSecretKey(bs58.decode(PRIVATEKEY));
  const swapUxdAmount = new BN(5000000);
...
export const uxdToSolInstruction = ({
  ...
  const keys = [
    ...
    { pubkey: new PublicKey(WSOLACCOUNT), isSigner: false, isWritable: true },
    { pubkey: new PublicKey(UXDACCOUNT), isSigner: false, isWritable: true },
    {
